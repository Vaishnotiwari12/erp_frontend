...
export default function ManageAlumniPage() {
  ...
  const columns = useMemo(() => [
    ...
    { accessorKey: 'passing_year', header: 'Passing Year' },
    { accessorKey: 'course', header: 'Course', cell: ({ row }) => <Badge variant="secondary">{row.original.course}</Badge> },
    { accessorKey: 'current_occupation', header: 'Occupation' },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Total Alumni" value={stats.total_alumni ?? 0} icon={GraduationCap} accent="primary" />
        <StatCard label="Active" value={stats.active ?? 0} icon={GraduationCap} accent="success" />
        <StatCard label="Upcoming Events" value={stats.upcoming_events ?? 0} icon={GraduationCap} accent="chart2" />
        <StatCard label="Completed Events" value={stats.completed_events ?? 0} icon={GraduationCap} accent="chart3" />
      </div>
          ...
          <div className="space-y-6">
            ...
            <dl className="grid grid-cols-2 gap-x-6 gap-y-4">
              {[
                ...
                { label: 'Passing Year', value: viewRow.passing_year },
                { label: 'Course', value: viewRow.course },
                { label: 'Current Occupation', value: viewRow.current_occupation },
      ...
      <DeleteDialog
        ...
        onConfirm={() => deleteAlumni(deleteRow._id)}
      />
    </div>
  )
}
...
function AlumniFormDrawer({ open, onOpenChange, title, initial, onSubmit }) {
  const [form, setForm] = useState({
    ...
    email: initial?.email || '',
    phone: initial?.phone || '',
    passing_year: initial?.passing_year || new Date().getFullYear(),
        ...
        <FormSection columns={1}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input type="number" min="1980" max="2099" value={form.passing_year} onChange={(e) => set('passing_year', parseInt(e.target.value) || new Date().getFullYear())} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Course</Label>
              <Input value={form.course} onChange={(e) => set('course', e.target.value)} placeholder="e.g. Class 12 - Science" />
            </div>
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Current Occupation</Label>
            <Input value={form.current_occupation} onChange={(e) => set('current_occupation', e.target.value)} placeholder="e.g. Software Engineer at Google" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Address</Label>
            <Input value={form.address} onChange={(e) => set('address', e.target.value)} placeholder="Current address" />
          </div>
          <div className="space-y-1.5">
