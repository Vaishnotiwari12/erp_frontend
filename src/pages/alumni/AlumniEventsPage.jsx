      ...
      cell: ({ row }) => (
        <button className="flex items-center gap-3 text-left" onClick={() => setViewRow(row.original)}>
          ...
          <div className="flex flex-col">
            <span className="font-medium hover:underline">{row.original.event_name}</span>
            <span className="text-xs text-muted-foreground">{formatDate(row.original.event_date)}</span>
          </div>
        </button>
      ),
    },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Total Events" value={rows.length} icon={CalendarDays} accent="primary" />
        <StatCard label="Upcoming" value={stats.upcoming_events ?? 0} icon={CalendarDays} accent="success" />
        <StatCard label="Completed" value={stats.completed_events ?? 0} icon={CalendarDays} accent="chart3" />
        <StatCard label="Total Alumni" value={stats.total_alumni ?? 0} icon={CalendarDays} accent="chart2" />
      </div>
      ...
      <Drawer
        ...
        title="Event Details"
        description={viewRow?.event_name}
        width="sm:max-w-md"
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3 rounded-xl border bg-muted/30 p-4">
              ...
              <div className="flex-1">
                <p className="font-semibold">{viewRow.event_name}</p>
                <p className="text-xs text-muted-foreground">{formatDate(viewRow.event_date)}</p>
              </div>
      ...
      <DeleteDialog
        ...
        entityName={deleteRow?.event_name}
    ...
    <Drawer
      ...
      footer={
        <DrawerFooter
          ...
          submitDisabled={!form.event_name.trim() || !form.event_date}
      ...
      <form onSubmit={(e) => { e.preventDefault(); onSubmit(form) }} className="space-y-4">
        <FormSection columns={1}>
          <div className="space-y-1.5">
            ...
            <Input value={form.event_name} onChange={(e) => set('event_name', e.target.value)} placeholder="e.g. Annual Alumni Meet 2024" required />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label className="text-xs">Event Date <span className="text-destructive">*</span></Label>
