  ...
  const columns = useMemo(() => [
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-3">
          ...
          <span className="font-medium">{row.original.content_title}</span>
        </div>
      ),
    },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Total Contents" value={stats.total_contents ?? 0} icon={Share2} accent="chart2" />
        <StatCard label="Total Videos" value={stats.total_videos ?? 0} icon={Share2} accent="chart3" />
        <StatCard label="Content Types" value={stats.total_types ?? 0} icon={Share2} accent="chart4" />
      </div>
...
function ShareListFormDrawer({ open, onOpenChange, title, contents, classes, onSubmit }) {
  ...
  const handleContentChange = (contentId) => {
    const c = contents.find((x) => x._id === contentId)
    setForm((f) => ({ ...f, content_id: contentId, content_title: c?.title || '' }))
  }

  const handleClassChange = (classId) => {
    const c = classes.find((x) => x.id === classId)
    setForm((f) => ({ ...f, class_id: classId, class_name: c?.name || '' }))
  }
      ...
      <form onSubmit={(e) => { e.preventDefault(); onSubmit(form) }} className="space-y-4">
        <FormSection columns={1}>
          <div className="space-y-1.5">
            ...
            <select value={form.content_id} onChange={(e) => handleContentChange(e.target.value)}
          ...
          <div className="space-y-1.5">
            ...
            <textarea value={form.note} onChange={(e) => set('note', e.target.value)} placeholder="Sharing note" rows={3}
