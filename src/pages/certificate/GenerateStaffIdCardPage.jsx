...
import { useGeneratedStaffIdCards } from '@/hooks/useCertificate'
...
export default function GenerateStaffIdCardPage() {
      ...
      cell: ({ row }) => (
        <button className="flex items-center gap-3 text-left" onClick={() => setViewRow(row.original)}>
          ...
          <div className="flex flex-col">
            <span className="font-medium hover:underline">{row.original.staff_id}</span>
  ...
  const rowActions = (r) => [
    ...
    { label: 'Print', icon: Printer, onClick: () => toast({ title: 'Printing…', description: r.staff_id }) },
    { label: 'Download', icon: Download, onClick: () => toast({ title: 'Downloading…', description: r.staff_id }) },
    { separator: true },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <Drawer
        ...
        description={viewRow?.staff_id}
        width="sm:max-w-md"
        footer={
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => toast({ title: 'Printing…', description: viewRow.staff_id })}>
              <Printer className="mr-2 h-4 w-4" /> Print
            </Button>
            <Button variant="outline" onClick={() => toast({ title: 'Downloading…', description: viewRow.staff_id })}>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3 rounded-xl border bg-muted/30 p-4">
              ...
              <div className="flex-1">
                <p className="font-semibold">{viewRow.staff_id}</p>
            ...
            <dl className="grid grid-cols-2 gap-x-6 gap-y-4">
              {[
                { label: 'Staff ID', value: viewRow.staff_id },
...
function GenerateStaffIdCardFormDrawer({ open, onOpenChange, onSubmit }) {
  ...
  return (
    <Drawer
      ...
      description="Enter staff and design details to generate"
      width="sm:max-w-md"
      footer={
        <DrawerFooter
          onCancel={() => onOpenChange(false)}
          submitLabel="Generate"
          submitDisabled={!form.staff_id.trim() || !form.design_id.trim()}
      ...
      <form onSubmit={(e) => { e.preventDefault(); onSubmit(form) }} className="space-y-4">
        <FormSection columns={1}>
          <div className="space-y-1.5">
            <Label className="text-xs">Staff ID <span className="text-destructive">*</span></Label>
            <Input value={form.staff_id} onChange={(e) => set('staff_id', e.target.value)} placeholder="Staff ObjectId" required />
          </div>
          <div className="space-y-1.5">
