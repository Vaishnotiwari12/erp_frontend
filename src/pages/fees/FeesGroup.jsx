...
export default function FeesGroupPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <PageHeader title="Fees Group" description="Organize fee types into logical groups." icon={Layers} actions={<><ImportButton onImport={() => toast({ title: 'Import started' })} /><Button onClick={() => setAddOpen(true)}><Plus className="mr-2 h-4 w-4" /> Add Group</Button></>} />
      ...
      <Drawer open={!!viewRow} onOpenChange={(o) => !o && setViewRow(null)} title="Group Details" description={viewRow?.name} width="sm:max-w-md" footer={<Button variant="outline" onClick={() => setViewRow(null)}>Close</Button>}>
        {viewRow ? (
          <dl className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
            {[{ label: 'Group Name', value: viewRow.name }, { label: 'Code', value: viewRow.code }, { label: 'Description', value: viewRow.description }, { label: 'Status', value: <StatusBadge status={viewRow.status} /> }, { label: 'Created', value: formatDate(viewRow.createdAt) }].map((r) => (
...
function GroupDrawer({ open, onOpenChange, title, initial, onSubmit }) {
  const [form, setForm] = useState({ name: initial?.name || '', code: initial?.code || '', description: initial?.description || '', status: initial?.status || 'active' })
  return (
    <Drawer open={open} onOpenChange={onOpenChange} title={title} description="Fee group details" width="sm:max-w-md" footer={<DrawerFooter onCancel={() => onOpenChange(false)} submitLabel={initial ? 'Save' : 'Create'} onSubmit={() => onSubmit(form)} />}>
      <form onSubmit={(e) => { e.preventDefault(); onSubmit(form) }} className="space-y-4">
        <FormSection columns={2}>
          <div className="space-y-1.5"><Label className="text-xs">Group Name <span className="text-destructive">*</span></Label><Input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="e.g. Tuition Fees" required /></div>
          <div className="space-y-1.5"><Label className="text-xs">Code <span className="text-destructive">*</span></Label><Input value={form.code} onChange={(e) => setForm((f) => ({ ...f, code: e.target.value }))} placeholder="e.g. TUI" required /></div>
