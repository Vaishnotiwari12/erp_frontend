...
function studentMultipart(input) {
  ...
  Object.entries(payload).forEach(([key, item]) => {
    if (key === 'name' || key === 'guardian' || key === 'transport' || key === 'hostel') {
  ...
  if (input.student_photo) form.append('student_photo', input.student_photo)
...
export const studentService = {
  ...
  list: (params = {}) => apiClient.get('/student/details/all', { params }),
  get: (id) => apiClient.get(`/student/details/${id}`),
  create: (payload) => {
    // Always send as JSON if no files, only use FormData if files are present
  ...
  update: (id, payload) => apiClient.put(`/student/details/${id}`, studentPayload(payload)),
  remove: (id) => apiClient.delete(`/student/details/${id}`),
